<!-- uniapp vue3 markdown解析 @itemclick="handleItemClick" -->
<template>
  <view class="ua__markdown"
    ><rich-text
      space="nbsp"
      user-select="true"
      :nodes="parseNodes(source)"></rich-text
  ></view>
</template>
...
const parseNodes = (value) => {
  ...
  // 修改为
  value = value.replace(/function/g, "func_tion"); // 使用安全的替代词而不是完全删除
  ...
  // htmlString = htmlString.replace(/<img/g, `<br /><img style="max-width:30%" /> <br />`)
  htmlString = htmlString.replace(
    /<img/g,
    `<br /><img  style="max-width:100%;display:block;"`
  );
